...
def test(image, weights_path):
    normalized_images, image_scales = mask_rcnn_preprocess(image)
    image_size = normalized_images.shape
    image_scale = norm_boxes_graph(image_scales, image_size[:2])
    model = mask_rcnn_detection(image_size=image_size, window=image_scale)
    model.load_weights(weights_path)
    input_image = tf.expand_dims(normalized_images, axis=0)
    out = model(input_image)
    out = out.numpy()
    det_image, detections, class_map_idx = mask_rcnn_postprocess(
        model, out[0], None, image)
...
weights_path = weights_path + folder + 'mask_rcnn.h5'
loader = LoadImage()
