...
from dext.model.marine_debris_ssd_mobilenet.ssd_mobilenet import SSD_MobileNet


@gin.configurable
def marine_debris_ssd_mobilenet(weight_path, backbone_folder):
    model = SSD_MobileNet(12, weight_folder=backbone_folder)
    model.load_weights(weight_path)
    return model


def marine_debris_ssd_mobilenet_preprocess(image, image_size=96,
                                           only_resize=False):
...
def marine_debris_ssd_mobilenet_postprocess(
