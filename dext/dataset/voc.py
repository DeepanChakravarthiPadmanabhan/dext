...
class VOC(Loader):
    ...
    def __init__(self, path=None, split='train', class_names='all',
        ...
        if class_names == 'all':
            self._class_names = voc_class_names
    ...
    def _load_VOC(self, dataset_name, split):
        self.parser = VOCParser(dataset_name, split, self._class_names,
                                self.with_difficult_samples, self.path,
                                self.evaluate, self.continuous_run,
                                self.result_dir)
...
class VOCParser(object):
    ...
    def __init__(self, dataset_name='VOC2007', split='train',
                 ...
                 evaluate=False, continuous_run=False, result_dir=None):
        ...
        if self.continuous_run:
            self.load_ran_ids = filter_image_ids(result_dir)
    ...
    def check_availability(self, image_index):
        if self.continuous_run:
            if int(image_index) in self.load_ran_ids:
                return 'available'
            else:
