...
from dext.evaluator.analyze_saliency_maps import deletion_realistic_errors
from dext.evaluator.analyze_saliency_maps import deletion_bounded_errors
from dext.evaluator.analyze_saliency_maps import insertion_realistic_errors
from dext.evaluator.analyze_saliency_maps import insertion_bounded_errors
...
def get_metrics(detection, raw_image_path, saliency, image_index, object_index,
                ...
                save_modified_images, image_adulteration_method, eval_deletion,
                eval_insertion, eval_ap_explain, coco_result_file, model=None):
    if eval_deletion:
        LOGGER.info('Deletion metrics for explaining %s & box offset %s'
        ...
        eval_metrics = deletion_bounded_errors(
        ...
        write_record(df_class_conf, 'del_bound_class', result_dir)
        ...
        write_record(df_class_flip_entry, 'del_real_class_flip', result_dir)
        ...
        write_record(df_class_conf_real, 'del_real_class', result_dir)

        df_reg_error_pixels = [str(image_index), object_index,
                               num_pixels_flipped, explaining, boxoffset, ]
        df_reg_error_pixels = df_reg_error_pixels + real_reg_error_pixels
        write_record(df_reg_error_pixels, 'del_real_error_pixels', result_dir)

        df_reg_error_iou = [str(image_index), object_index,
                            num_pixels_flipped, explaining, boxoffset, ]
        df_reg_error_iou = df_reg_error_iou + real_reg_error_iou
        write_record(df_reg_error_iou, 'del_real_error_iou', result_dir)

        df_x_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_x_error_pixels = df_x_error_pixels + real_x_error_pixels
        write_record(df_x_error_pixels, 'del_real_x', result_dir)

        df_y_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_y_error_pixels = df_y_error_pixels + real_y_error_pixels
        write_record(df_y_error_pixels, 'del_real_y', result_dir)

        df_w_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_w_error_pixels = df_w_error_pixels + real_w_error_pixels
        write_record(df_w_error_pixels, 'del_real_w', result_dir)

        df_h_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_h_error_pixels = df_h_error_pixels + real_h_error_pixels
        write_record(df_h_error_pixels, 'del_real_h', result_dir)

    if eval_insertion:
        LOGGER.info('Insertion metrics for explaining %s & box offset %s' %
                    (explaining, boxoffset))
        eval_metrics = analyze_saliency_maps(
            detection, raw_image_path, saliency)
        iou, centroid, variance = eval_metrics
        eval_metrics = insertion_bounded_errors(
            saliency, raw_image_path, detection, image_size, model_name,
            ap_curve_linspace, explain_top5_backgrounds,
            save_modified_images, image_adulteration_method, result_dir, model,
            object_index)
        ...
        df_class_conf = [str(image_index), object_index, explaining,
                         boxoffset, ]
        df_class_conf = df_class_conf + bound_class_conf
        write_record(df_class_conf, 'ins_bound_class', result_dir)

        df_reg_error_pixels = [str(image_index), object_index, explaining,
                               boxoffset, ]
        df_reg_error_pixels = df_reg_error_pixels + bound_reg_error_pixels
        write_record(df_reg_error_pixels, 'ins_bound_error_pixels', result_dir)

        df_reg_error_iou = [str(image_index), object_index, explaining,
                            boxoffset, ]
        df_reg_error_iou = df_reg_error_iou + bound_reg_error_iou
        write_record(df_reg_error_iou, 'ins_bound_error_iou', result_dir)

        df_x_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_x_error_pixels = df_x_error_pixels + bound_x_error_pixels
        write_record(df_x_error_pixels, 'ins_bound_x', result_dir)

        df_y_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_y_error_pixels = df_y_error_pixels + bound_y_error_pixels
        write_record(df_y_error_pixels, 'ins_bound_y', result_dir)

        df_w_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_w_error_pixels = df_w_error_pixels + bound_w_error_pixels
        write_record(df_w_error_pixels, 'ins_bound_w', result_dir)

        df_h_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_h_error_pixels = df_h_error_pixels + bound_h_error_pixels
        write_record(df_h_error_pixels, 'ins_bound_h', result_dir)

        eval_metrics = insertion_realistic_errors(
            saliency, raw_image_path, detection, image_size, model_name,
            ap_curve_linspace, explain_top5_backgrounds,
            save_modified_images, image_adulteration_method, result_dir, model)
        (num_pixels_flipped, real_class_conf, real_reg_error_pixels,
        ...
        df_class_flip_entry = [str(image_index), object_index, explaining,
                               ...
                               detection.score, detection.class_name,
                               iou, centroid, variance, num_pixels_flipped]
        write_record(df_class_flip_entry, 'ins_real_class_flip', result_dir)

        df_class_conf = [str(image_index), object_index,
                         num_pixels_flipped, explaining, boxoffset, ]
        df_class_conf_real = df_class_conf + real_class_conf
        write_record(df_class_conf_real, 'ins_real_class', result_dir)

        df_reg_error_pixels = [str(image_index), object_index,
                               num_pixels_flipped, explaining, boxoffset, ]
        df_reg_error_pixels = df_reg_error_pixels + real_reg_error_pixels
        write_record(df_reg_error_pixels, 'ins_real_error_pixels', result_dir)

        df_reg_error_iou = [str(image_index), object_index,
                            num_pixels_flipped, explaining, boxoffset, ]
        df_reg_error_iou = df_reg_error_iou + real_reg_error_iou
        write_record(df_reg_error_iou, 'ins_real_error_iou', result_dir)

        df_x_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_x_error_pixels = df_x_error_pixels + real_x_error_pixels
        write_record(df_x_error_pixels, 'ins_real_x', result_dir)

        df_y_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_y_error_pixels = df_y_error_pixels + real_y_error_pixels
        write_record(df_y_error_pixels, 'ins_real_y', result_dir)

        df_w_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_w_error_pixels = df_w_error_pixels + real_w_error_pixels
        write_record(df_w_error_pixels, 'ins_real_w', result_dir)

        df_h_error_pixels = [str(image_index), object_index, explaining,
                             boxoffset, ]
        df_h_error_pixels = df_h_error_pixels + real_h_error_pixels
        write_record(df_h_error_pixels, 'ins_real_h', result_dir)
...
def merge_all_maps(detection, image_path, saliency_list, image_index,
                   ...
                   merge_method, coco_result_file, model=None):
    ...
    get_metrics(detection, image_path, combined_map, image_index, object_index,
                ...
                ap_curve_linspace, result_dir, explain_top5_backgrounds,
                save_modified_images, image_adulteration_method, eval_deletion,
                eval_insertion, eval_ap_explain, coco_result_file, model)
